...
import SuggestionPopup from "../Components/Popups/Suggestion-Popup"
import DonationPopup from "../Components/Popups/Donation-Popup"
import AllSurveysPopup from "../Components/Questionnaire/QuestionnairePopup" // New component for all surveys
import { Link } from "react-router-dom"
import axios from "axios"
import { useAuth } from "../contexts/AuthContext"
...
function Home() {
  ...
  // States for All Surveys Popup
  const [allUnansweredSurveys, setAllUnansweredSurveys] = useState([]) // All surveys user needs to answer
  const [showAllSurveysPopup, setShowAllSurveysPopup] = useState(false)
  const [loadingAllSurveys, setLoadingAllSurveys] = useState(true) // Tracks initial survey load
  const [errorAllSurveys, setErrorAllSurveys] = useState(null)
  const [loadingUserDetails, setLoadingUserDetails] = useState(true)
  ...
  // Fetch ALL unanswered surveys for user and show them all at once
  useEffect(() => {
    const fetchAllUnansweredSurveys = async () => {
      setLoadingAllSurveys(true)
      setErrorAllSurveys(null)
      ...
      // Check if user has already completed all surveys in this session
      const hasCompletedAllSurveys = localStorage.getItem(`hasCompletedAllSurveys_${currentMemberId}`)
      if (hasCompletedAllSurveys === "true") {
        setLoadingAllSurveys(false)
        setShowAllSurveysPopup(false)
        return
      }
      ...
      try {
        ...
        if (response.data && Array.isArray(response.data.data)) {
          const surveys = response.data.data
          setAllUnansweredSurveys(surveys)

          if (surveys.length > 0) {
            // Show all surveys at once
            setShowAllSurveysPopup(true)
          } else {
            // No unanswered surveys, keep popup closed
            setShowAllSurveysPopup(false)
            // Mark as completed since there are no surveys to answer
            localStorage.setItem(`hasCompletedAllSurveys_${currentMemberId}`, "true")
          }
        } else {
          console.log("No unanswered surveys available or invalid format.")
          setAllUnansweredSurveys([])
          setShowAllSurveysPopup(false)
          // Mark as completed since there are no surveys
          localStorage.setItem(`hasCompletedAllSurveys_${currentMemberId}`, "true")
        }
      } catch (err) {
        console.error("Error fetching unanswered surveys:", err)
        // Only set error if it's not an auth issue
        if (err.response?.status !== 401) {
          setErrorAllSurveys("Failed to load surveys. Please try again.")
        }
        setAllUnansweredSurveys([])
        setShowAllSurveysPopup(false)
      } finally {
    ...
    // Trigger survey loading only when user details are available
    if (!authLoading && isAuthenticated && token && userDetails?.id && !loadingUserDetails) {
      fetchAllUnansweredSurveys()
    }
  }, [authLoading, isAuthenticated, token, userDetails, loadingUserDetails])

  // Callback from AllSurveysPopup when user completes all surveys
  const handleAllSurveysCompleted = () => {
    console.log("User completed all surveys")

    // Mark surveys as completed for this user
    if (userDetails?.id) {
      localStorage.setItem(`hasCompletedAllSurveys_${userDetails.id}`, "true")
    }
    ...
    alert("Thank you for completing all surveys! Your responses have been submitted successfully.")
  }

  // Callback when user closes surveys without completing
  const handleAllSurveysClose = () => {
    setShowAllSurveysPopup(false)
    // Don't mark as completed if user just closes without finishing
  }
  ...
  return (
    <DashboardLayout userDetails={userDetails}>
      ...
      {/* All Surveys Popup - Show all surveys at once */}
      {loadingAllSurveys ? (
        // Optionally show a small loading indicator for survey logic
        <div className="sr-only">Loading surveys for user...</div>
      ) : (
        allUnansweredSurveys.length > 0 && ( // Only render if surveys are loaded and available
          <AllSurveysPopup
            show={showAllSurveysPopup}
            onClose={handleAllSurveysClose}
            onComplete={handleAllSurveysCompleted}
            surveysData={allUnansweredSurveys}
          />
        )
      )}
    </DashboardLayout>
